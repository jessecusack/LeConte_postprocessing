...
##nc metadata
create_nc_from_adp_sentinel <- function(adp, name){
  if (!inherits(adp, "adp")){
  ...
  # vdepthdim <- ncdim_def("vdistance", "m", as.double(vdist))
  ...
  dlname <- "upward velocity from upward beam"
  wv_def <- ncvar_def("wv", "m/s", list(timedim, depthdim, londim, latdim), FillValue, dlname, prec = "float")
  ...
  dlname <- "echo intensity upward beam"
  bv_def <- ncvar_def("eiv", "a", list(timedim, depthdim, londim, latdim), FillValue, dlname, prec = "float")
  
  dlname <- "correlation_beam_1"
  q1_def <- ncvar_def("q1", "a", list(timedim, depthdim, londim, latdim), FillValue, dlname, prec = "float") 
  
  dlname <- "correlation_beam_2"
  q2_def <- ncvar_def("q2", "a", list(timedim, depthdim, londim, latdim), FillValue, dlname, prec = "float") 
  
  dlname <- "correlation_beam_3"
  q3_def <- ncvar_def("q3", "a", list(timedim, depthdim, londim, latdim), FillValue, dlname, prec = "float") 
  
  dlname <- "correlation_beam_4"
  q4_def <- ncvar_def("q4", "a", list(timedim, depthdim, londim, latdim), FillValue, dlname, prec = "float") 
  
  dlname <- "correlation vertical beam"
  qv_def <- ncvar_def("qv", "a", list(timedim, depthdim, londim, latdim), FillValue, dlname, prec = "float") 
  ...
  ncout <- nc_create(ncfname, list(u_def, v_def, w_def, wv_def, e_def, b1_def, b2_def, 
                                   b3_def, b4_def, bv_def, q1_def, q2_def, q3_def, 
                                   q4_def, qv_def, p_def, r_def, head_def, 
                                   t_def, pr_def, D_def), force_v4 = TRUE)
  ...
  ncvar_put(ncout, bv_def, adp[['va', 'numeric']])
  ...
  ncvar_put(ncout, qv_def, adp[['vq', 'numeric']])
